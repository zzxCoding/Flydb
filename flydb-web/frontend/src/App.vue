...
const data = ref<Bootstrap>(), selected = ref(localStorage.getItem('flydb.selected') ?? ''), config = ref<ConfigSnapshot>()
const tab = ref<'migrations' | 'connection' | 'history'>('migrations'), query = ref(''), sidebar = ref(false)
...
const temporaryPasswords = ref<Record<string, string>>({}), discardTarget = ref<string>(), advanced = ref<'baseline' | 'repair' | 'remove'>(), baselineVersion = ref('1')
...
const groups = computed(() => {
  ...
  for (const p of data.value?.profiles ?? []) {
    if (![p.name, p.environment, p.configPath, p.group].join(' ').toLowerCase().includes(query.value.toLowerCase())) continue
    const key = p.group || t('ungrouped'); result.set(key, [...(result.get(key) ?? []), p])
  }
  return result
})
...
const pending = computed(() => migrations.value.filter(m => ['PENDING', 'OUTDATED', 'UNDONE'].includes(m.state ?? '')).length)
const viewedRun = computed(() => data.value?.runs.find(run => run.id === activeRun.value))
const viewedPlan = computed(() => data.value?.runs.find(run => run.id === previewRun.value))
const date = (value?: string) => value ? new Date(value).toLocaleString(locale.value, { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' }) : '—'
...
async function refresh() {
  ...
  fetching = true
  try {
    data.value = await api<Bootstrap>('/bootstrap'); serviceError.value = ''
...
async function action(command: string, extra: Record<string, unknown> = {}) {
  ...
  try {
    ...
    else if (['migrate', 'undo', 'baseline', 'repair'].includes(command)) activeRun.value = run.id
...
async function confirmAdvanced() {
  ...
  } else if (advanced.value) await action(advanced.value, { baselineVersion: baselineVersion.value, confirmed: true })
}
...
<NConfigProvider :theme="dark ? darkTheme : null">
  <div class="workbench-shell">
    <aside class="sidebar" :class="{ visible: sidebar }">
      ...
      <nav class="profile-list" :aria-label="t('configurations')"><section v-for="([group, profiles]) in groups" :key="group"><h3>{{ group }}<span>{{ profiles.length }}</span></h3><button v-for="p in profiles" :key="p.id" class="profile-row" :aria-current="selected === p.id ? 'page' : undefined" @click="choose(p.id)"><span class="db-glyph"><Icon name="database" :size="17" /></span><span class="profile-title">{{ p.name }}<small>{{ p.environment || p.configPath.split(/[\\/]/).slice(-2).join('/') }}</small></span><span v-if="data?.runs.some(r => r.status === 'RUNNING' && (r.profileId === p.id || r.configPath === p.configPath))" class="activity-dot" /></button></section><div v-if="query && !groups.size" class="empty-small"><p>{{ t('noMatch') }}</p><button class="text-button" @click="query = ''">{{ t('clearSearch') }}</button></div></nav>
    ...
    <main class="main-workspace" :inert="sidebar || undefined">
      ...
      <div v-else class="workspace-content">
        ...
        <div v-if="running" class="running-strip"><span class="activity-dot" /><span>{{ t('running') }} · {{ running.script || t(running.command) }}</span><button class="text-button" @click="activeRun = running.id">{{ t('viewRun') }}<Icon name="arrow" :size="15" /></button></div>
        ...
        <section v-if="tab === 'migrations'" class="migration-view">
          ...
          <div v-if="inspection" class="migration-summary"><div><span>{{ t('currentVersion') }}</span><strong class="mono">{{ inspection.current ?? '—' }}</strong></div><div><span>{{ t('pending') }}</span><strong class="accent">{{ pending.toLocaleString(locale) }}</strong></div><div><span>{{ t('applied') }}</span><strong>{{ migrations.filter(m => ['SUCCESS', 'BASELINE', 'OUT_OF_ORDER'].includes(m.state ?? '')).length.toLocaleString(locale) }}</strong></div></div>
          <div v-if="migrations.length" class="table-scroll"><table class="migration-table"><thead><tr><th>{{ t('version') }}</th><th>{{ t('script') }}</th><th>{{ t('state') }}</th></tr></thead><tbody><tr v-for="m in migrations" :key="`${m.script}-${m.state}`"><td><span class="version-tag">{{ m.version ?? 'R' }}</span></td><td><span class="script-name"><Icon name="file" :size="16" /><code>{{ m.script }}</code></span><small>{{ m.description }}</small></td><td><Status :value="m.state ?? 'UNKNOWN'" /></td></tr></tbody></table></div>
        ...
        <section v-if="tab === 'history'" class="run-history"><div class="section-heading"><h2>{{ t('history') }}</h2></div><button v-for="run in runs" :key="run.id" class="run-row" @click="activeRun = run.id"><span class="run-icon"><Icon name="history" /></span><span><strong>{{ te(run.command) ? t(run.command) : run.command }}</strong><small>{{ date(run.startedAt) }} · {{ run.source }}</small></span><Status :value="run.status" /><Icon name="chevron" /></button><div v-if="!runs.length" class="migration-empty"><Icon name="history" :size="32" /><h3>{{ t('noHistory') }}</h3><p>{{ t('noHistoryBody') }}</p></div></section>
      </div>
    </main>
  </div>
  <NModal v-model:show="allHistory">      <section class="dialog-surface run-dialog run-history">
        ...
        <button v-for="run in data?.runs" :key="run.id" class="run-row" @click="activeRun = run.id"><span class="run-icon"><Icon name="history" /></span><span><strong>{{ run.profileName || run.configPath || 'Flydb' }}</strong><small>{{ te(run.command) ? t(run.command) : run.command }} · {{ date(run.startedAt) }} · {{ run.source }}</small></span><Status :value="run.status" /><Icon name="chevron" /></button>
        <p v-if="!data?.runs.length" class="migration-empty">{{ t('noHistory') }}</p>
      </section>
</NModal>
  ...
  <NModal :show="!!viewedRun" @update:show="value => { if (!value) activeRun = undefined }"><div class="dialog-surface run-dialog"><RunPanel v-if="viewedRun" :run="viewedRun" @close="activeRun = undefined" /></div></NModal>
  <NModal :show="!!viewedPlan" :mask-closable="false" @update:show="value => { if (!value) previewRun = undefined }"><div class="dialog-surface plan-dialog"><PlanPanel v-if="viewedPlan" :run="viewedPlan" :busy="submitting" @close="previewRun = undefined" @execute="action(viewedPlan.command === 'undo-plan' ? 'undo' : 'migrate', { planId: viewedPlan.result?.planId })" /></div></NModal>
  ...
  <NModal :show="!!advanced" :mask-closable="false"><div class="dialog-surface"><template v-if="advanced"><h2>{{ t(advanced) }}</h2><p>{{ t(advanced === 'remove' ? 'removeBody' : advanced === 'baseline' ? 'baselineBody' : 'repairBody') }}</p><p class="target-line"><code>{{ config?.effective['flydb.url'] }}</code></p><label v-if="advanced === 'baseline'" class="field"><span>{{ t('baselineVersion') }}</span><input v-model="baselineVersion" required /></label><footer class="dialog-actions"><button class="button" @click="advanced = undefined">{{ t('cancel') }}</button><button class="button primary" :disabled="submitting" @click="confirmAdvanced">{{ t('confirmOperation') }}</button></footer></template></div></NModal>
</NConfigProvider>
</template>
