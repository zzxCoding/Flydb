package com.flydb.runtime.output;
...
public final class SecretRedactor {
    ...
    public static String redact(String text) {
        ...
        return text.replaceAll("(?i)(password[=:]\\s*)[^\\s]+", "$1****")
                .replaceAll("(?i)([?&;](?:token|access[-_]?token|secret|api[-_]?key|credential)=)[^&;\\s#]*", "$1****")
                .replaceAll("(?i)(jdbc:(?:oracle|oceanbase):thin:[^/@\\s]+/)[^@\\s]+@", "$1****@")
                .replaceAll("(?i)(//[^/:@\\s]+:)[^@\\s]+@", "$1****@");
    }

    public static String redactSecret(String text, String password) {
        String redacted = redact(text);
        if (redacted != null && password != null && !password.isEmpty()) {
