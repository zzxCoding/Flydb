...
- [x] 语句级周期进度：向诊断通道报告当前脚本、已确认完成数、总语句数、耗时与速率；保持 `--json` stdout 单行契约不变
- [x] 失败执行快照：区分事务模式、已确认成功、已回滚和驱动无法可靠定位的批次范围，不把推算值或未知数据库状态描述成已提交事实
- [x] 补齐 OceanBase-Oracle 实测限制：组合 `MODIFY` 应拆分类型与可空性变更；登录用户与 `CURRENT_SCHEMA` 不同时说明 `USER_*` 字典视图风险及 `ALL_* + owner` 写法
...
## 0.3.5：本机图形工作台（已完成用户验收）

通过 `flydb web` 打开可离线使用的图形界面，管理一套或多套现有配置，完成检查、
预览和迁移，并查看 Web / 同机 CLI 的执行记录。人和 Agent 共用原有配置文件，
没有模型时也能完整操作。易用性优先，视觉沿用 Flydb 官网，支持中英文切换。

这是单纯的本机 GUI：不做账户、登录页、角色权限、团队或审批管理。设计、范围和
验收记录见 [Web 工作台设计](docs/design/12-web-workbench.md)。用户验收通过，纳入 0.3.5 发布。

## 阶段四：存量变更智能（0.4.0 起）

回答“改这个字段，究竟会伤到什么”。针对运行多年、依赖散落在数据库对象与应用代码里的存量系统。

独立试用入口：[flydb-analysis Skill](flydb-skills/docs/flydb-analysis.md)（0.2.0 preview）。已提供方言预检、快照与漂移、数据库依赖、应用引用和综合影响分析的基础工作流，支持离线材料；分析报告独立于 CLI/JAR 发布。当前范围与样例证据见 [验证记录](flydb-skills/evals/flydb-analysis/VALIDATION.md)。以下正式阶段条目仍按各自契约和验证范围验收，Skill 预览版不等同于 CLI 原生诊断或完整数据库覆盖。
...
- 通用数据库 GUI / SQL IDE（本机 Flydb 配置与迁移 GUI 见上述 0.3.x 范围）
- ORM 或应用开发框架
