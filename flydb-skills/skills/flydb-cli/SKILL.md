...
当 dry-run 语句数、历史耗时或远程库特征表明 `migrate` 可能超过当前终端、SSH 或 Agent 工具调用的存活时间时，进入「长迁移」分支：先读取[CLI 命令参考的「长时间迁移与后台运行」](../../../docs/reference/commands.md#长时间迁移与后台运行)，再用其 `nohup` 包装器启动已授权的迁移，记录绝对运行目录、日志、PID 和 `exit-code` 路径。工具调用超时或暂时无新日志时，保持现有迁移进程并改用有界状态检查；取消或终止进程需要用户明确授权。只有完成标记已出现、进程已结束且退出码可读时，才进入第 5 步；PID 消失但无完成标记时按「结果未知」保留现场，不自动重放或修改历史。

如果迁移失败，先读取错误码、数据库原始错误和诊断通道中的“迁移失败执行快照”，按失败阶段、事务模式、JDBC 已确认执行数、定位可信度和事务结果判断现场。“确认执行”不等于已提交；只有快照明确显示“已回滚”时才能按整体回滚处理，非事务、回滚失败或提交结果未知时先核对数据库现状。不要自动重放迁移或执行 `repair`；后者会修改历史表，必须在用户确认修复策略后执行。
