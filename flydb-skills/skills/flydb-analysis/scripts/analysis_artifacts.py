...
def compare(before, after, before_path="baseline.json", after_path="observed.json"):
    ...
    def unknown(gap, next_check, affected=None):
        existing = next((item for item in report["unknowns"]
                         if item["gap"] == gap and item["nextCheck"] == next_check), None)
        if existing is None:
            report["unknowns"].append({"gap": gap, "affectedClaims": affected or [], "nextCheck": next_check})
        else:
            existing["affectedClaims"] = sorted(set(existing["affectedClaims"]) | set(affected or []))
        report["analysisStatus"] = "partial"

    def scope_label(scope):
        return json.dumps({p: scope[p] for p in (*KEY[:-1], "names")},
