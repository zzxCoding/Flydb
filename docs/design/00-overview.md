...
**2026-09-06 补充决策**：按用户确认，增加可选的 `flydb web` 本机 GUI。普通 CLI
仍独立运行，不要求启动服务；Web 复用同一迁移内核，只监听回环地址，界面打开即用。
不增加用户账户、登录、角色权限或审批管理。该可选 GUI 不恢复旧原型的必选服务形态，
具体契约见 [12 Web 工作台](12-web-workbench.md)。
