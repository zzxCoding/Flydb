...
Agent 自动化入口见 [`flydb-skills`](../../flydb-skills/README.md)，其中的 [`flydb-cli`](../../flydb-skills/skills/flydb-cli/SKILL.md) 按开放 `SKILL.md` 格式面向 Claude Code、OpenAI Codex、Gemini CLI、Kimi Code、ZCode、Hermes Agent、Pi 等主流 Agent 复用；它只引用本目录和数据库上手文档，不另行维护命令参数。
