...
需要用图形界面管理配置与迁移时，阅读[本机 GUI 指南](web.md)（[English](web.en.md)，待发布）。
...
需要同时管理多个数据库家族、多套测试与生产环境时，阅读[多数据库多环境自动化](multi-environment.md)；把 Flydb 接入 GitHub Actions 或 Jenkins 时，阅读[CI 集成指南](ci-integration.md)；把 Flydb 挂进 Claude Code、Cursor 等 MCP 宿主时，阅读[MCP 接入指南](mcp-adapter.md)。
