...
**验证状态（2026-08-14）**：已在授权真实 Oracle 实例完成端到端验证——validate、`--dry-run migrate`、clean、migrate，覆盖 `CREATE OR REPLACE EDITIONABLE TRIGGER` 的 PL/SQL 块、目录版本族 `family-range` 范围迁移，以及 `${...}` 业务模板经 `--placeholder-replacement=false` 原样入库。该轮真库验证暴露并已修复三处方言缺陷：历史表 DDL 的 `DEFAULT`/`NOT NULL` 顺序（ORA-00907）、clean 序列与回收站发现缺失（残留对象导致 ORA-00955 撞名）、PL/SQL 块探测器不识别 `EDITIONABLE`（孤儿 `END;` 报 ORA-00900）。验证证据不等于厂商认证。
