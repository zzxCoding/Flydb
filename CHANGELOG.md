...
- `flydb web` 本机图形工作台：一条 CLI 命令启动，支持中英文、明暗主题、多配置导入/发现/复制/分组，以及迁移检查、预览、执行进度和结果报告；无需账号、角色体系或大模型，离线发行包运行只需 Java 8+。
- 配置表单与完整文件编辑共用同一份原文件，支持 Oracle 服务名/SID、CodeMirror 行号/高亮/补全/查找替换、快捷保存、格式检查、外部修改对照和草稿保护。新建项目共用 `flydb init` 的完整模板与初始化逻辑，明确展示配置、V1 示例和驱动说明，创建本身不执行 SQL。
- Web 与同机 CLI 共用执行记录；记录脚本进度、JDBC 确认数、事务结果及执行后核验。CLI `--json` 仍保持单行信封，记录与诊断不进入 stdout。
- Core 提供可选执行观察器与已准备计划执行入口，Web 在执行边界校验配置、脚本与预览计划，复用原迁移引擎和状态规则。

### 修复与实现

- Safari 等新浏览器打开普通本机地址时自动建立同源连接，保留回环地址、Origin 和临时会话校验。
- 修复关闭执行详情、迁移预览或高级操作后遗留页面滚动锁的问题；嵌套弹窗全部关闭后恢复滚动。
- CLI 共用配置、驱动、脚手架和结果渲染下沉到 Java 8 的 `flydb-runtime`，图形界面位于独立 `flydb-web`；Core 保持 Java 8 与零第三方运行时依赖。源码完整构建新增 Node.js 22.12+ 要求，发行包不需要 Node.js。
- 随包 Agent Skill 补充 Web 版本要求、启动与共用配置流程、隐私及数据库写入边界；新增对应评测用例，插件版本同步为 0.3.5。MCP Adapter 独立版本保持 0.1.0，工具协议不变。
- 更新 MCP 间接依赖 `qs` 至 6.16.0，修复依赖审计发现的拒绝服务漏洞。

### 验证边界

- GUI 在授权的临时 MySQL 环境完成配置、预览、迁移、撤销、基线、修复及执行记录验收；Oracle 本轮仅覆盖连接表单和配置文件行为，不代表新增 Oracle 实库认证。
- 文件编辑与初始化经过原文件读回、非法保存阻断、外部冲突、断连恢复、双语/主题和窄屏验收；滚动问题通过真实浏览器与 App/NModal 回归验证。
- Web 不提供 clean；GUI 启动和配置保存不扩大数据库写入授权。执行过程跨进程中断后保留未知/中断状态，不能据此自动重放 SQL。
